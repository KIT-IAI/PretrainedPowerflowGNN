...
class DER_loss(nn.Module):
    ...
    def __init__(self, regularize=True, regcoeff=1):
        ...
        self.regularize = regularize
        self.regcoeff = regcoeff
        self.der_mask = torch.zeros(5)
        self.der_mask[-1] = 1

    def forward(self, output, target):
        ### Working stock code
        #masked_target = torch.masked_select(target, self.der_mask.type(torch.bool))
        #loss = self.criterion(output, masked_target)
        #loss = 0.1 * loss + 0.9 * torch.abs(output[self.der_bus] - target[self.der_bus][-1])

        ### Lasse:
        lamb = 0.9
        mse = torch.mean((output - target[:,-2]) ** 2)
        indices = torch.nonzero(target[:, -1] == 1).squeeze()
        mse_ehub = torch.mean((output[indices] - target[indices, -2]) ** 2)
        loss = (1 - lamb) * mse + lamb * mse_ehub

        # Try out only ehub node
        loss = mse_ehub
